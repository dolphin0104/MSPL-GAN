...
def img_gen_lr(img, scale, interp='cubic'):
    if img.ndim == 2:
        ih, iw = img.shape
    elif img.ndim == 3:
        ih, iw, _ = img.shape
    ...
    ih = ih//scale
    iw = iw//scale
    return cv2.resize(img, dsize=(ih, iw), interpolation=interp)
...
def get_randomcrop(img_in, crop_size=(112, 96)):
    img = np.copy(img_in)
    if img.ndim == 2:
        ih, iw = img.shape
    elif img.ndim == 3:
        ih, iw, _ = img.shape
    assert ih >= crop_size[0], 'Crop sizes is larger than image size'
    assert iw >= crop_size[1], 'Crop sizes is larger than image size'       
    ix = random.randrange(0, iw - crop_size[1] + 1)
    iy = random.randrange(0, ih - crop_size[0] + 1)
    img = img[iy:iy + crop_size[0], ix:ix + crop_size[1], :]
    return img
...
def add_noise(img_in, noise_type='G', noise_val=0.01):
    if noise_type == 'G':
...
def add_blur(img_in, blur_type='G', blur_ksize=25, blur_val=100):
    if blur_type == 'G':

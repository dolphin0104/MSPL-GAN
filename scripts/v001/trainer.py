...
from model.Generatros import FocusResNet, FocusRRDBNet
from model.Discriminators import Discriminator_112
from loss.arcface_extractor import Arcface_Extractor
from loss.gan_loss import GAN_Loss
from loss.vgg_extractor import VGG16_FeatureExtractor, VGG19_FeatureExtractor, gram_matrix
from utils.weight_initializer import init_weights
from utils.train_helpers import timer, tensor2img, bgr2ycbcr, make_dirs, write_log
from utils.metrics import calculate_psnr, calculate_ssim
from loss.sphereface import SphereFace_Extractor
...
class Trainer():
    def __init__(self, args):
        ...
        # ===================================================================
        # 3. build Generator
        if args.Model['Generator']['model_type'] == 'ResNet':
            self.netG = FocusResNet(**args.Model['Generator'])
        elif args.Model['Generator']['model_type'] == 'RRDBNet':
            self.netG = FocusRRDBNet(**args.Model['Generator'])           
        ...
        self.print_network(self.netG, args.Model['Generator']['model_type'])        
        ...
        if args.Losses['l_cont_weight'] != 0:
            ...
            self.netVGG = VGG19_FeatureExtractor(
                is_img=True, use_bn=False, use_input_norm=True, 
        ...
        if args.Losses['l_face_weight'] != 0:
            ...
            # self.cri_cos = nn.CosineEmbeddingLoss().to(self.device) 
        ...
        for epoch in range(start_epoch, total_epochs): 
            ...
            for iters, (gt_data, in_data, _) in enumerate(self.loader_train):
                ...
                if self.l_img_w:
                    ...
                    self.log_dict['loss_img'] = loss_img.item()
                ...
                if self.l_adv_w:                        
                    ...
                    self.log_dict['loss_adv_img'] = loss_adv_i.item()
                ...
                if self.l_face_w:                                        
                    ...
                    loss_face = self.l_face_w * \
                        self.cri_l1(fake_feat, real_feat)
                    self.log_dict['loss_face'] = loss_face.item()
                    loss_G_total += loss_face                    

                self.log_dict['loss_G_total'] = loss_G_total.item()
                ...
                if self.l_adv_w:                    
                    for p in self.netD.parameters():
                        ...
                        self.log_dict['loss_netD_real'] = l_netD_real.item()
                    
                        l_netD_fake = self.cri_gan(pred_d_fake - torch.mean(pred_d_real), False)
                        self.log_dict['loss_netD_fake'] = l_netD_fake.item()

                        loss_netD_total = (l_netD_real + l_netD_fake) / 2
                        self.log_dict['loss_netD_total'] = loss_netD_total.item()
                ...
                self.print_loss(
                    t_per_iter, self.log_dict, epoch, iters, total_epochs, total_iters)

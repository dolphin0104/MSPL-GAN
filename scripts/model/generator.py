...
class GALNet_v2(nn.Module):
    def __init__(self, 
        ...
        self.net0 = simpleUshapeNet(in_channels, n_feats, n_feats, n_blocks[0], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh, res_scale=res_scale)
        self.tail0 = nn.Conv2d(n_feats, out_channels, 3, 1, 1)
        
        self.net1 = simpleUshapeNet(n_feats, n_feats, n_feats, n_blocks[1], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh, res_scale=res_scale)
        self.tail1 = nn.Conv2d(n_feats, out_channels, 3, 1, 1)

        self.net2 = simpleUshapeNet(n_feats, n_feats, n_feats, n_blocks[2], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh, res_scale=res_scale)
        self.tail2 = nn.Conv2d(n_feats, out_channels, 3, 1, 1)
        
        self.net3 = simpleUshapeNet(n_feats, n_feats, n_feats, n_blocks[3], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh, res_scale=res_scale)        
...
class GALNet_v3(nn.Module):
    def __init__(self, 
        ...
        self.head0 = nn.Conv2d(in_channels, n_feats, 3, 1, 1)
        self.net0 = simpleUshapeNet(in_channels, n_feats, n_feats, n_blocks[0], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh, res_scale=res_scale)
        self.tail0 = nn.Conv2d(n_feats, out_channels, 3, 1, 1)
    ...
    def forward(self, x): 
        ...
        dense_feat0 = feat0 + feat
        feat1 = self.net1(dense_feat0)
        dense_feat1 = dense_feat0 + feat1
        feat2 = self.net2(dense_feat1)
        dense_feat2 = dense_feat1 + feat2
        feat3 = self.net3(dense_feat2)
        dense_feat3 = dense_feat2 + feat3 

        out0 = torch.tanh(self.tail0(dense_feat0)) 
        out1 = torch.tanh(self.tail1(dense_feat1)) 
        out2 = torch.tanh(self.tail2(dense_feat2)) 
        out3 = torch.tanh(self.tail3(dense_feat3))   
        return out0, out1, out2, out3 


class GALNet_v3_BASE(nn.Module):
    def __init__(self, 
                 in_channels=3, 
                 out_channels=3, 
                 n_feats=128, 
                 ...
                 use_global_residual=True, 
                 use_tanh=False):
        super(GALNet_v3_BASE, self).__init__()
        ...
        # self.net4 = simpleUshapeNet(n_feats, n_feats, n_feats, n_blocks[4], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh)        
        # self.to_rgb4 = nn.Conv2d(n_feats, out_channels, (1, 1), bias=True) 
...
class GALNet_v3_GR(nn.Module):
    def __init__(self, 
                 ...
                 use_global_residual=True, 
                 use_tanh=False):
        super(GALNet_v3_GR, self).__init__()
...
class GALNet_v3_DENSE(nn.Module):
    def __init__(self, 
                 in_channels=3, 
                 out_channels=3, 
                 n_feats=128, 
                 ...
                 use_global_residual=True, 
                 use_tanh=False):
        super(GALNet_v3_DENSE, self).__init__()
    ...
    def forward(self, x):  
        ...
        feat0 = self.net0(feat)
        out_feat0 = feat0 + feat
        
        feat1 = self.net1(out_feat0)
        out_feat1 = feat1 + feat0 + feat
        
        feat2 = self.net2(out_feat1)
        out_feat2 = feat2 + feat1 + feat0 + feat
        
        feat3 = self.net3(out_feat2) 
        out_feat3 = feat3 + feat2 + feat1 + feat0 + feat

        # feat4 = self.net4(out_feat3)
        # out_feat4 = feat4 + feat3 + feat2 + feat1 + feat0 + feat
        

        out0 = torch.tanh(self.to_rgb0(out_feat0)) 
        out1 = torch.tanh(self.to_rgb1(out_feat1)) 
        out2 = torch.tanh(self.to_rgb2(out_feat2)) 
        out3 = torch.tanh(self.to_rgb3(out_feat3))
        # out4 = torch.tanh(self.to_rgb3(out_feat4))   
        return out0, out1, out2, out3  


class GALNet_v4_DENSE(nn.Module):
    def __init__(self, 
                 in_channels, 
                 out_channels, 
                 n_feats=64, 
                 ...
                 use_global_residual=True, 
                 use_tanh=False):
        super(GALNet_v4_DENSE, self).__init__()
...
class JSH_UshapeNet(nn.Module):
    def __init__(self, 
                 ...
                 use_tanh=True,
                 res_scale=1):
        super(JSH_UshapeNet, self).__init__()
        self.use_global_residual = use_global_residual
        # self.use_tanh = use_tanh
...
class JSHNet_v3_BASE(nn.Module):
    def __init__(self, 
                 in_channels=3, 
                 out_channels=3, 
                 n_feats=64, 
                 ...
                 use_global_residual=True, 
                 use_tanh=False):
        super(JSHNet_v3_BASE, self).__init__()
        
        self.from_rgb = nn.Conv2d(in_channels, n_feats, (1, 1), bias=True) 

        self.net0 = JSH_UshapeNet(n_feats, n_feats, n_feats, n_blocks[0], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh)
        self.to_rgb0 = nn.Conv2d(n_feats, out_channels, (1, 1), bias=True) 
        
        self.net1 = JSH_UshapeNet(n_feats, n_feats, n_feats, n_blocks[1], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh)
        self.to_rgb1 = nn.Conv2d(n_feats, out_channels, (1, 1), bias=True) 

        self.net2 = JSH_UshapeNet(n_feats, n_feats, n_feats, n_blocks[2], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh)
        self.to_rgb2 = nn.Conv2d(n_feats, out_channels, (1, 1), bias=True) 
        
        self.net3 = JSH_UshapeNet(n_feats, n_feats, n_feats, n_blocks[3], norm_type, act_type, use_channel_attention, use_global_residual, use_tanh)        
        self.to_rgb3 = nn.Conv2d(n_feats, out_channels, (1, 1), bias=True)
